...
function input(e) {
  if (e.value.length == 4) {
    ...
    document.getElementById("ressult").innerHTML =
      `<div class='row pt-2 border-bottom border-right border-left' id='${NumberId}'>
        <div class='col-6 fs-4'>${e.value}</div>
        <div class='col-6 fs-4'>${temp}<button class="btn btn-danger float-end" onclick="close1(${NumberId})">X</button></div>
